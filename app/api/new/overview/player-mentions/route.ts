...
function parseSentimentValue(value: string | number | undefined) {
...
async function mapCommentsToPlayers(
  ...
  for (const batch of commentBatches) {
    ...
    try {
      ...
      console.error("[player-mentions] Failed to parse OpenAI response", error)
      continue
    }
  }
...
function aggregateSentiment(
  ...
  nameToNumber: Map<string, string | undefined>,
): PlayerAggregate[] {
  ...
  for (const comment of comments) {
    ...
    if (pos === null || neg === null || neu === null) continue
  ...
  for (const [name, data] of totals.entries()) {
    ...
    aggregates.push({
      name,
      number: nameToNumber.get(name),
      positive: Number((data.pos / data.count).toFixed(2)),
      neutral: Number((data.neu / data.count).toFixed(2)),
      negative: Number((data.neg / data.count).toFixed(2)),
      count: data.count,
    })
  }
...
function pickTopPositiveAndNegative(aggregates: PlayerAggregate[]) {
  if (!aggregates.length) return { topPositive: null, topNegative: null }

  const sortedByPositive = [...aggregates].sort(
    (a, b) => b.positive - a.positive,
  )
  const sortedByNegative = [...aggregates].sort(
    (a, b) => b.negative - a.negative,
  )

  return {
    topPositive: sortedByPositive[0] || null,
    topNegative: sortedByNegative[0] || null,
  }
}
...
export async function GET() {
  try {
    ...
    const nameToNumber = new Map<string, string | undefined>()
    players.forEach((p) => nameToNumber.set(p.name, p.number))
    ...
    const { topPositive, topNegative } = pickTopPositiveAndNegative(aggregates)

    const now = new Date()
    const windowStart = new Date(Date.now() - TWO_WEEKS_MS)
    ...
    console.error("[player-mentions] Failed to process", error)
    return NextResponse.json(
      { error: "Failed to process player mentions" },
      { status: 500 },
    )
  }
}
